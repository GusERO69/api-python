...
def predict(user_id):
    ...
    future_dates = pd.date_range(start=end_date + timedelta(days=1), periods=7 * 24, freq='H')
    total_dates = total_dates.append(future_dates)
    ...
    try:
        ...
        cursor = conn.cursor()
        insert_query = """
        INSERT INTO results (user_id, prediction, date_week, higher, probability, error, real_data) 
        ...
        for prediction in predictions_json:
            ...
            cursor.execute(insert_query, (user_id, pred, date, higher, proba, err, real))
        conn.commit()
